...
7. **【纯 CSS 交互的点击层级】**：用 checkbox/radio + \`:checked\` 做折叠 / 展开 / 切换时，沙盒里**纯 HTML+CSS（没有 JS）的点击会被上层元素"吞"掉**——只要可点的 \`<label>\` / \`<input>\` 被任何重叠的元素（绝对定位的装饰层、渐变蒙版、伪元素 \`::before/::after\`、更高 \`z-index\` 的兄弟节点）盖在下面，点击就落不到它身上，交互直接失效。所以：
