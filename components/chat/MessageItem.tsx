...
const MessageItem = React.memo(({
    ...
    const renderAvatar = (src: string) => (
        shouldShowAvatar ? (
            <div className={`relative ${avatarSizeClass} shrink-0`}>
                <img
                    src={src}
                    className={`w-full h-full ${avatarRadiusClass} object-cover relative z-0`}
                    alt="avatar"
                    loading="lazy"
                    decoding="async"
                />
    ...
    const containerStyle: React.CSSProperties = {
        ...
        opacity: 1,
        borderRadius: '4px',
        boxShadow: isUser ? 'none' : '0 1px 1px rgba(0,0,0,0.03)',
        border: 'none',
    };

    // 气泡垂直内边距统一
    const bubblePadding = 'px-2 py-2';
    ...
    return commonLayout(
        <div className={`relative ${bubblePadding} animate-fade-in active:scale-[0.98] transition-transform overflow-visible ${isUser ? 'sully-bubble-user' : 'sully-bubble-ai'}`}
            ...
            <div
                ...
                style={{
                    ...
                    filter: isUser ? 'none' : 'drop-shadow(1px 1px 1px rgba(0,0,0,0.03))',
                }}
            />
